import React, { useState, useEffect } from 'react';
import { useCartStore } from '../../store/cartStore';
import { Button } from '../ui/Button';
import { Input } from '../ui/Input';
import { ScrollReveal } from '../ui/ScrollReveal';
import { Chrome } from 'lucide-react';

export const Register: React.FC = () => {
  const { login, user, navigateTo } = useCartStore();

  const [fullName, setFullName] = useState('');
  const [email, setEmail] = useState('');
  const [password, setPassword] = useState('');
  const [agree, setAgree] = useState(true);
  const [errorMsg, setErrorMsg] = useState('');
  const [loading, setLoading] = useState(false);

  useEffect(() => {
    if (user) {
      navigateTo('/account');
    }
  }, [user]);

  const handleGoogleSignIn = async () => {
    setLoading(true);
    setErrorMsg('');
    try {
      const { auth, googleAuthProvider, signInWithPopup } = await import('../../lib/firebase.ts');
      await signInWithPopup(auth, googleAuthProvider);
      navigateTo('/account');
    } catch (err: any) {
      console.error(err);
      setErrorMsg(err.message || 'Failed to authenticate with Google.');
    } finally {
      setLoading(false);
    }
  };

  const handleSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    if (!fullName.trim()) {
      setErrorMsg('Please supply your full legal name.');
      return;
    }
    if (!email || !email.includes('@')) {
      setErrorMsg('Please supply a valid liaison email.');
      return;
    }
    if (!password || password.length < 6) {
      setErrorMsg('Password should contain at least 6 characters.');
      return;
    }
    if (!agree) {
      setErrorMsg('You must agree to the Maison luxury circle guidelines.');
      return;
    }

    setErrorMsg('');
    login(email, fullName);
    navigateTo('/account');
  };

  return (
    <div className="bg-neutral-950 text-white min-h-screen pt-32 pb-24 font-sans select-none flex items-center justify-center">
      <div className="w-full max-w-md px-6">
        
        <ScrollReveal>
          <div className="border border-neutral-900 bg-neutral-900/10 p-8 md:p-10 rounded shadow-2xl flex flex-col space-y-8">
            
            <div className="text-center space-y-2">
              <span className="text-[10px] uppercase tracking-[0.3em] text-[#c9a96e] font-semibold">
                ESTABLISH CLIENT RECORD
              </span>
              <h1 className="font-serif text-2xl md:text-3xl font-semibold tracking-tight text-white">
                Atelier Registration
              </h1>
              <p className="text-[11px] text-neutral-500 font-sans tracking-widest uppercase">
                Register to receive private collection capsules
              </p>
            </div>

            <div className="flex flex-col space-y-4">
              <button
                type="button"
                onClick={handleGoogleSignIn}
                disabled={loading}
                className="w-full flex items-center justify-center space-x-3 py-3 px-4 border border-neutral-800 bg-neutral-900/40 hover:bg-neutral-900 hover:border-neutral-700 rounded transition-all text-xs font-mono uppercase tracking-widest outline-none disabled:opacity-50"
              >
                <Chrome size={14} className="text-[#c9a96e]" />
                <span>{loading ? 'Decrypting Atelier...' : 'Register with Google Portal'}</span>
              </button>
              
              <div className="relative flex py-2 items-center">
                <div className="flex-grow border-t border-neutral-900"></div>
                <span className="flex-shrink mx-4 text-[9px] uppercase tracking-widest text-neutral-600 font-mono">Or Manual Entry</span>
                <div className="flex-grow border-t border-neutral-900"></div>
              </div>
            </div>

            <form onSubmit={handleSubmit} className="space-y-5">
              
              <Input
                label="Full Legal Name"
                value={fullName}
                onChange={(e) => {
                  setFullName(e.target.value);
                  setErrorMsg('');
                }}
                placeholder="Christian Audigier"
              />

              <Input
                label="Liaison Email Address"
                type="email"
                value={email}
                onChange={(e) => {
                  setEmail(e.target.value);
                  setErrorMsg('');
                }}
                placeholder="atelier@noir.vip"
              />

              <Input
                label="Choose Secure Password"
                type="password"
                value={password}
                onChange={(e) => {
                  setPassword(e.target.value);
                  setErrorMsg('');
                }}
                placeholder="at least 6 characters"
              />

              {/* Secure terms check */}
              <div className="flex items-center space-x-3 text-xs text-neutral-400 select-none font-sans py-2">
                <input
                  type="checkbox"
                  id="agree-checkbox"
                  checked={agree}
                  onChange={(e) => setAgree(e.target.checked)}
                  className="accent-[#c9a96e] h-4 w-4 bg-transparent rounded border-neutral-800"
                />
                <label htmlFor="agree-checkbox" className="leading-tight cursor-pointer">
                  I agree to Maison Noir's <strong className="text-white">Circle Code of Conduct</strong> and privacy terms.
                </label>
              </div>

              {errorMsg && (
                <p className="text-[10px] uppercase tracking-widest text-red-500">{errorMsg}</p>
              )}

              <Button
                variant="gold"
                type="submit"
                className="w-full text-glow-light"
              >
                Establish Client Record
              </Button>
            </form>

            <div className="border-t border-neutral-900 pt-6 text-center space-y-3">
              <button
                onClick={() => navigateTo('/auth/login')}
                className="text-xs text-[#c9a96e] hover:text-white transition-colors uppercase tracking-widest font-mono font-medium outline-none text-glow-light"
              >
                Access Salon Account Log Instead
              </button>
            </div>

          </div>
        </ScrollReveal>

      </div>
    </div>
  );
};
